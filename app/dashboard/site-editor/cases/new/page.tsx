...
export default function NewCasePage() {
  ...
  async function save() {
    ...
    const res = await fetch("/api/site-editor/cases", {
      method: "POST",
    ...
    setSaving(false);
    if (res.ok) {
      router.push("/dashboard/site-editor/cases");
    } else {
    ...
    <div className="max-w-2xl pb-32">
      ...
      <div className="space-y-5">
        ...
        <div className="bg-white rounded-2xl border p-5 space-y-4">
          ...
          <div>
            ...
            <input className={inputCls} value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Sweee Branding" />
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-1">Industry</label>
            <input className={inputCls} value={industry} onChange={(e) => setIndustry(e.target.value)} placeholder="Food & Beverage" />
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-1">Short Description (shown in grid)</label>
            <textarea className={`${inputCls} resize-none`} rows={2} value={shortDesc} onChange={(e) => setShortDesc(e.target.value)} />
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-1">Full Description (case page)</label>
        ...
        <div className="bg-white rounded-2xl border p-5">
          <h2 className="font-semibold mb-3">Cover Image <span className="text-xs text-gray-400">(thumbnail in grid)</span></h2>
          {coverImage ? (
        ...
        {/* Hero Image */}
        <div className="bg-white rounded-2xl border p-5">
          <h2 className="font-semibold mb-3">Hero Image <span className="text-xs text-gray-400">(top of case page)</span></h2>
          {heroImage ? (
            <div className="relative">
        ...
        <div className="bg-white rounded-2xl border p-5">
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-semibold">Gallery <span className="text-xs text-gray-400">(inside case page)</span></h2>
            <button onClick={() => galleryRef.current?.click()} className="text-sm border px-3 py-1 rounded-lg hover:bg-gray-50">
              + Add images
            </button>
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t p-4">
        <button
          onClick={save}
          disabled={saving}
          className="w-full bg-black text-white rounded-full py-4 text-sm font-medium disabled:opacity-50"
        >
